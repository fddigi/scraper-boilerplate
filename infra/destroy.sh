...
destroy_worker() {
  log_step "1/3 Cloudflare Worker (and its secrets + KV binding)"
  if [[ "$CONFIRMED" != "true" ]]; then
...
destroy_turso_database() {
  log_step "2/3 Turso database (deleting the db revokes all of its auth tokens)"
  if [[ "$CONFIRMED" != "true" ]]; then
...
clear_repo_secrets() {
  ...
  for secret in TURSO_DB_NAME CF_WORKER_NAME CLOUDFLARE_ACCOUNT_ID; do
    if [[ "$CONFIRMED" != "true" ]]; then
